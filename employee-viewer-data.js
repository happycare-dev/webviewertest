...
(function (EV) {
  ...
  window.receiveUiCapabilities = function () {
    ...
    if (hasAccessCode(parsed.recordAccess) && hasAccessCode(parsed.layoutAccess)) {
      state.canEditDelete = hasFileMakerEditAccess(parsed.recordAccess, parsed.layoutAccess);
    } else {
